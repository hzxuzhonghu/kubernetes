...
func newTestBlockingWebhook(t *testing.T, endpoint string, groupVersion schema.GroupVersion) *blockingBackend {
	return newWebhook(t, endpoint, groupVersion).(*blockingBackend)
}

func newWebhook(t *testing.T, endpoint string, groupVersion schema.GroupVersion) audit.Backend {
	config := v1.Config{
	...
	backend, err := NewBackend(f.Name(), groupVersion, false)
